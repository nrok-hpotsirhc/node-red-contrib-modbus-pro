...
| 2026-06-08 | MS-8 | Complete | RTU live hardware test hardened for Windows PowerShell/COM11: opt-in via `RTU_LIVE_TESTS=true`, added `test:rtu-live` script with direct Mocha invocation to avoid npm path issues with `&` in workspace paths, and COM11 connection smoke test verified 3/3 passing. Main suite remains 646/646 passing |
| 2026-06-08 | MS-8 | Complete | Added opt-in `test:rtu-com11` FC23 smoke test for COM11, 19200 baud, 8E1, unit ID 4: reads holding registers 1-2 while writing/verifying registers 3-4. Script isolation and skip behavior verified; active hardware write test left for manual execution |
