...
describe('RtuTransport', function () {
  ...
  beforeEach(function () {
    ...
    // Stub modbus-serial prototype methods used by RtuTransport
  ...
  describe('constructor', function () {
    it('should create an instance with default config', function () {
      const transport = new RtuTransport();
      expect(transport).to.be.instanceOf(RtuTransport);
      expect(transport).to.be.instanceOf(EventEmitter);
      expect(transport._config.serialPort).to.equal('/dev/ttyUSB0');
      expect(transport._config.baudRate).to.equal(9600);
      expect(transport._config.parity).to.equal('none');
      expect(transport._config.dataBits).to.equal(8);
      expect(transport._config.stopBits).to.equal(1);
    ...
    it('should merge custom config with defaults', function () {
      ...
      expect(transport._config.dataBits).to.equal(8); // default preserved
    });
    ...
    it('should have type "rtu"', function () {
      const transport = new RtuTransport();
      expect(transport.type).to.equal('rtu');
    });

    it('should expose serialAvailable property', function () {
      const transport = new RtuTransport();
      expect(typeof transport.serialAvailable).to.equal('boolean');
    });
  });

  // ---- serialport availability ----

  describe('serialport availability', function () {
    it('isSerialPortAvailable should return a boolean', function () {
      const result = RtuTransport.isSerialPortAvailable();
      expect(typeof result).to.equal('boolean');
    });
  });

  // ---- connect() when serialport is available ----

  describe('connect() with serialport available', function () {
    let transport;

    beforeEach(function () {
      transport = new RtuTransport({ serialPort: '/dev/ttyUSB0', baudRate: 9600 });
      // Force serialAvailable to true for these tests
      transport._serialAvailable = true;
    });
    ...
    it('should connect successfully and emit "connect"', async function () {
      ...
      expect(ModbusRTU.prototype.connectRTUBuffered.calledOnce).to.be.true;
      expect(ModbusRTU.prototype.connectRTUBuffered.firstCall.args[0]).to.equal('/dev/ttyUSB0');
      expect(ModbusRTU.prototype.connectRTUBuffered.firstCall.args[1]).to.deep.equal({
        baudRate: 9600,
        parity: 'none',
        dataBits: 8,
        stopBits: 1
    ...
    it('should be a no-op when already connected', async function () {
      await transport.connect();
      ModbusRTU.prototype.connectRTUBuffered.resetHistory();

      await transport.connect();
      expect(ModbusRTU.prototype.connectRTUBuffered.called).to.be.false;
    });

    it('should throw and emit "error" on connection failure', async function () {
      const connError = new Error('Cannot open /dev/ttyUSB0');
      ModbusRTU.prototype.connectRTUBuffered.rejects(connError);
      ...
      try {
        ...
        expect(err.message).to.equal('Cannot open /dev/ttyUSB0');
      }
  ...
  // ---- connect() when serialport is unavailable ----

  describe('connect() without serialport', function () {
    it('should throw a descriptive error when serialport is not installed', async function () {
      const transport = new RtuTransport();
      transport._serialAvailable = false;
      ...
      try {
        ...
        expect(err.message).to.include('serialport package is not installed');
      }
  ...
  describe('disconnect()', function () {
    ...
    it('should be a no-op when not connected', async function () {
      const fresh = new RtuTransport();
      const disconnSpy = sandbox.spy();
      fresh.on('disconnect', disconnSpy);
  ...
  describe('read operations', function () {
    ...
    it('should reject when not connected', async function () {
      const disconnected = new RtuTransport();
      const ops = [
      ...
      for (const p of ops) {
        try {
          ...
          expect(err.message).to.equal('RtuTransport: not connected');
        }
      }
    });
  });
